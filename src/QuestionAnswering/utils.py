...
def postprocess_qa_predictions(
    ...
    features_per_example = collections.defaultdict(list) # 기본값이 빈 리스트([])인 dictionary
    for i, feature in enumerate(features):
        features_per_example[example_id_to_index[feature["example_id"]]].append(i)
    # id가 같은 example을 한 list에 인덱스 번호로 저장한다.
    ...
    for example_index, example in enumerate(tqdm(examples)):
        ...
        for feature_index in feature_indices:
            ...
            # Optional : `token_is_max_context`, 제공되는 경우 현재 feature에서 사용할 수 있는 max context가 없는 answer를 제거합니다
            ...
            if (
                ...
                min_null_prediction = {
                    "offsets": (0, 0), # cls 토큰 -> 즉, 정답이 없다고 판단하는 경우
            ...
            # n_best_size 만큼 가장 큰 값들의 index를 가져온다.
            ...
            for start_index in start_indexes:
                for end_index in end_indexes:
                    ...
                    if (
                        ...
                        # -> context sequence가 아니라면 None으로 설정해두었다.
                    ):
                        continue
                    ...
                    if (
                        ...
                        # token_is_max_context : {"0":True} 라면 continue하게 된다.
                    ):
                        continue
...
def check_no_error(
    ...
    config_max_seq_length = config.dataQA.tokenizer.max_seq_length()
    if config_max_seq_length > tokenizer.model_max_length:
        logger.warn(
            f"The max_seq_length passed ({config_max_seq_length}) is larger than the maximum length for the"
            f"model ({tokenizer.model_max_length}). Using max_seq_length={tokenizer.model_max_length}."
        )
    max_seq_length = min(config_max_seq_length, tokenizer.model_max_length)
